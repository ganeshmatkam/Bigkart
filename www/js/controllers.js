...
app.controller('HomeCtrl', function ($scope, $timeout, $ionicSlideBoxDelegate, $ionicLoading, $ionicScrollDelegate, WC, Shop, Data) {
    ...
    $scope.doRefresh = function () {
        page = 1;
        WC.api().get('products?search=featured', function (err, data, res) {
            if (err) console.log(err);
...
app.controller('ProductCtrl', function ($scope, $state, $timeout, $filter, $cordovaToast, $cordovaSocialSharing, $window, $ionicModal, $ionicLoading, $ionicSlideBoxDelegate, $ionicScrollDelegate, $stateParams, WC, Shop) {
    ...
    $scope.addItem = function (x, qty) {
        ...
        if (cart) {
            ...
            if (!exist) {
                if (x.option)
                    cart.push({ id: x.id, title: x.title, price: x.price, img: x.featured_src, qty: qty, variations: { name: x.name, option: x.option } });
                else
                    cart.push({ id: x.id, title: x.title, price: x.price, img: x.featured_src, qty: qty });
                window.localStorage.setItem(Shop.name + "-cart", JSON.stringify(cart));
                $scope.showSuccess('Product added successfully');
            }
        } else {
...
app.controller('CategoryCtrl', function ($scope, $timeout, $ionicLoading, $ionicScrollDelegate, $stateParams, WC, $state, Shop, $ionicPopup, $cordovaToast) {
    ...
    $scope.gotoProduct = function (x) {
        $state.go('app.product', { id: x.id });
    }
    $scope.addItem = function (event, x, qty, isRemove) {
        if (event) {
        ...
        if (cart) {
            ...
            for (i in cart) {
                if (cart[i].id == x.id) {
                    ...
                    $scope.updateCart();
                    if (!isRemove) {
                        $cordovaToast.showLongBottom("Product added successfully");
                    }
                    else {
                        $cordovaToast.showLongBottom("Product removed successfully");
                    }
                    break;
                }
            }
    ...
    $scope.modifyQty = function (event, item, act) {
        if (event) {
            event.stopPropagation();
        }
        if (item.pqty) {
            if (act) {
                item.pqty++;
                $scope.addItem(null, item, 1, false);
            }
            else {
                item.pqty--;
                $scope.addItem(null, item, 1, true);
            }
        }
        else {
            if (act) {
    ...
    $scope.addToCart = function (event, x) {
        if (event) {
            event.stopPropagation();
        }
        $scope.data = {
        ...
        var myPopup = $ionicPopup.show({
            ...
            buttons: [
                {
                    text: '<i class="icon ion-close">',
                    type: 'button-stable button-clear'
                },
                {
                    text: '<i class="icon ion-checkmark">',
...
app.controller('AccountCtrl', function ($scope, $ionicLoading, $state, WC) {
    $ionicLoading.show();
...
app.controller('EditBillingCtrl', function ($scope, $http, $ionicModal, $ionicLoading, $state, WC) {
    $scope.u = {
        first_name: '',
        last_name: '',
        email: '',
...
app.controller('EditShippingCtrl', function ($scope, $http, $ionicModal, $ionicLoading, $state, WC) {
    $scope.u = {
        first_name: '',
        last_name: '',
        country: '',
...
app.controller('EditAccountCtrl', function ($scope, $ionicLoading, $state, WC) {
    $scope.u = {
        first_name: '',
        last_name: '',
        email: ''
